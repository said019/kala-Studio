...
const CampaignsPage = () => {
  ...
  const sendMutation = useMutation({
    ...
    mutationFn: async (confirm: boolean = false) =>
      (await api.post("/admin/campaigns/send", { name, segment, message, confirm })).data,
    onSuccess: (res) => {
      toast({
    ...
    onError: (err: any) => {
      ...
      if (err?.response?.status === 409 && data?.requiresConfirm) {
        const ok = window.confirm(
          `⚠️ Vas a enviar un WhatsApp a ${data.count} personas.\n\n` +
          `Esto es un envío masivo y no se puede deshacer. ¿Confirmas?`,
        );
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              ...
              <Button
                onClick={() => {
                  ...
                  if (window.confirm(`¿Enviar esta campaña por WhatsApp?\n\nUna vez enviada no se puede deshacer.`)) {
